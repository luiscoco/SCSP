...
type RecompCycle
    ...
    type(Compressor) :: mc, rc, pc1                          ! compressor and recompressor user-defined types
    type(HeatExchanger) :: LT, HT, PHX, RHX, PC, COOLER      ! heat exchanger user-defined types
    real(dp), dimension(14) :: temp, pres, enth, entr, dens  ! thermodynamic properties at the state points of the cycle (K, kPa, kJ/kg, kJ/kg-K, kg/m3)
...
subroutine calculate_turbomachine_outlet( &
    ...
    if (error_code /= 0) then
        ...
        error_trace%files(1) = 1
		print *, 'Luis Coco Error en la funcion CO2_PS calculation.'
        return
    end if
    ...
    if (error_code /= 0) then
		...
		print *, 'Luis Coco Error en la funcion CO2_PH calculation.'
