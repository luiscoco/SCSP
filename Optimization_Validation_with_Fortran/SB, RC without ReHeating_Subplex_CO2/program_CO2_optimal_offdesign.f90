...
!
!------------------------------------------------------------w
      subroutine carbondioxideoptimal(w_dot_net1,t_mc_in1,t_t_in1,p_mc_in1,p_mc_out1, &
                  ua_lt1,ua_ht1,eta_mc1,eta_rc1,eta_t1, &
                  ...
                  HT_Pcin,HT_Phin,HT_Pcout,HT_Phout,HT_Q,LT_UA,HT_UA, &
				  LT_Effc,HT_Effc,N_mc_design,t_mc_in_off,t_t_in_off, &
                  target_off,target_code_off,lowest_pressure_off, &
                  highest_pressure_off,recomp_frac_guess_off, &
                  fixed_recomp_frac_off,N_mc_guess_off,fixed_N_mc_off, &
                  N_t_guess_off,fixed_N_t_off,opt_tol_off1)

      !DEC$ ATTRIBUTES DLLEXPORT, ALIAS: 'carbondioxideoptimal'
      ...
      integer :: i
      logical , INTENT (INOUT):: fixed_recomp_frac_off,fixed_N_mc_off,fixed_N_t_off
      real(dp) , INTENT(INOUT):: w_dot_net1,t_mc_in1,t_t_in1,ua_lt1,N_mc_design,opt_tol_off1
      real(dp) , INTENT(INOUT):: t_mc_in_off,t_t_in_off,target_off,lowest_pressure_off
      real(dp) , INTENT(INOUT):: highest_pressure_off,recomp_frac_guess_off
      real (dp), INTENT(INOUT):: N_mc_guess_off,N_t_guess_off
      real(dp) , INTENT(INOUT):: ua_ht1,eta_mc1,eta_rc1,eta_t1,eta_thermal1
      ...
      integer , INTENT (INOUT):: n_sub_hxrs1,target_code_off
...
! Define and optimize a design point.
      ...
      call optimal_target_off_design(      &
           ...
           target = target_off,            &
           target_code = target_code_off,                &  ! [input] type of optimization: 1) target W_dot (max eta), 2) target Q_dot_PHX (max eta)
           N_sub_hxrs = n_sub_hxrs1,                &  ! number of sub-heat exchangers to use in each recuperator
           lowest_pressure = lowest_pressure_off,    &
           highest_pressure = highest_pressure_off,  &
           recomp_frac_guess = recomp_frac_guess_off,    & ! Could vary 
           fixed_recomp_frac = fixed_recomp_frac_off,    &
           N_mc_guess = N_mc_guess_off,        &
           fixed_N_mc = fixed_N_mc_off,           &
           N_t_guess = N_t_guess_off,            &
           fixed_N_t = fixed_N_t_off,             &
           tol = tol1,                &  ! convergence tolerance to use
           opt_tol = opt_tol_off1,            &
           error_trace = error_trace1       &
      )
		...
		recomp_frac_guess_off=recomp_cycle1%recomp_frac
		
        N_mc_guess_off = recomp_cycle1%mc%N
        N_t_guess_off = recomp_cycle1%t%N

      end subroutine carbondioxideoptimal
