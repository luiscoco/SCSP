...
subroutine design( &
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc1_in, T_mc2_in, T_t_in, P_mc1_in, P_mc2_in, P_mc1_out, P_mc2_out, P_rhx_in, T_rht_in, UA_LT, UA_HT, recomp_frac
    real(dp), intent(in) :: eta_mc1, eta_mc2, eta_rc, eta_t, eta_trh, tol
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC1,DP_PC2, DP_PHX, DP_RHX
    ...
    real(dp) :: w_mc1, w_mc2, w_rc, w_t, w_trh, C_dot_min, Q_dot_max
    ...
    real(dp) :: m_dot_t, m_dot_mc, m_dot_rc, eta_mc1_isen,eta_mc2_isen, eta_rc_isen, eta_t_isen, eta_trh_isen
    ...
    T8_loop: do T8_iter = 1,max_iter
        ...
        T9_loop: do T9_iter = 1,max_iter
                ...
                m_dot_t = W_dot_net / (w_mc1 * (1.0_dp - recomp_frac) +w_mc2 * (1.0_dp - recomp_frac) + w_rc * recomp_frac + w_t + w_trh)  ! required mass flow rate through turbine
    ...
    recomp_cycle%W_dot_net = w_mc1 * m_dot_mc + w_mc2 * m_dot_mc + w_rc * m_dot_rc + w_t * m_dot_t + w_trh * m_dot_t
...
subroutine optimal_design( &
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc1_in, T_mc2_in, T_t_in, P_mc1_in_guess, P_rhx_in_guess, T_rht_in, UA_rec_total, eta_mc1, eta_mc2, eta_rc, eta_t, eta_trh
    real(dp), intent(in) :: P_high_limit, P_mc2_out_guess, PR_mc2_guess, recomp_frac_guess, LT_frac_guess, tol, opt_tol
    logical, intent(in)  :: fixed_P_mc1_in, fixed_P_mc2_out, fixed_PR_mc2, fixed_recomp_frac, fixed_LT_frac, fixed_P_rhx_in
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT,DP_PC1, DP_PC2, DP_PHX, DP_RHX
    type(ErrorTrace), intent(out)  :: error_trace
    type(RecompCycle), intent(out) :: recomp_cycle
    ...
    real(dp) :: subplex_fmin, scale(max_free_vars), work(300)
    ...
    contains
        ...
        real(dp) function design_point_eta(n, x)
            ...
            real(dp) :: P_rhx_in_local, P_mc2_in_local, P_mc2_out_local, P_mc1_in_local, PR_mc2_local, recomp_frac_local,  LT_frac_local
            ...
            ! Check inputs.
            if (P_mc1_in_local < 100.0_dp) then  ! low-pressure limit
            ...
            if (P_mc1_in_local < 1000.0_dp) then
                design_point_eta = 0.0_dp
                return
            end if
