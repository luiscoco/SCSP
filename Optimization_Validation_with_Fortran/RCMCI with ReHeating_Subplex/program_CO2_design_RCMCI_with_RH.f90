...
!
!------------------------------------------------------------w
        subroutine carbondioxide(w_dot_net1,t_mc1_in1,t_mc2_in1,t_t_in1, &
        p_mc1_in1,p_mc1_out1, p_mc2_in1,p_mc2_out1,&
        p_rhx_in1,t_rht_in1,ua_lt1,ua_ht1, &
        eta_mc11,eta_mc21,eta_rc1,eta_t1,eta_trh1, &
        n_sub_hxrs1,recomp_frac1,tol1,eta_thermal1, &
        dp_lt1,dp_lt2,dp_ht1,dp_ht2,dp1_pc1,dp1_pc2,dp2_pc1,dp2_pc2, &
        dp_phx1,dp_phx2, dp_rhx1,dp_rhx2,temp1,temp2,temp3,temp4,temp5,temp6,temp7, &
      ...
      integer :: i
      real(dp) , INTENT(INOUT):: w_dot_net1,t_mc1_in1,t_mc2_in1,t_t_in1,ua_lt1
      real(dp) , INTENT(INOUT):: ua_ht1,eta_mc11,eta_mc21,eta_rc1,eta_t1,eta_trh1,eta_thermal1
      real(dp) , INTENT(INOUT):: recomp_frac1,tol1,p_mc1_in1,p_mc1_out1,p_mc2_in1,p_mc2_out1
      real(dp) , INTENT(INOUT):: p_rhx_in1,t_rht_in1,m_dot_turbine1,LT_UA,HT_UA,LT_Effc,HT_Effc	  
      real(dp) , INTENT(INOUT):: dp_lt1,dp_lt2,dp_ht1,dp_ht2,dp1_pc1,dp1_pc2,dp2_pc1,dp2_pc2,dp_phx1,dp_phx2,dp_rhx1,dp_rhx2
...
! Define and optimize a design point.
      call design(                         &
           ...
           T_mc2_in = t_mc2_in1,               &               ! turbine inlet temperature (K)
           T_t_in = t_t_in1,               &               ! turbine inlet temperature (K)
           P_mc1_in = p_mc1_in1,             &  ! [input] compressor inlet pressure (kPa)
           P_mc1_out = p_mc1_out1,           &  ! [input] compressor outlet pressure (kPa)
           P_mc2_in = p_mc2_in1,             &  ! [input] compressor inlet pressure (kPa)
           P_mc2_out = p_mc2_out1,           &  ! [input] compressor outlet pressure (kPa)
           P_rhx_in = p_rhx_in1,           & 
           T_rht_in = t_rht_in1,           &
           ...
           eta_mc1 = eta_mc11,               &  ! design-point efficiency of the main compressor; isentropic if positive, polytropic if negative
           eta_mc2 = eta_mc21,               &  ! design-point efficiency of the main compressor; isentropic if positive, polytropic if negative
         ...
         temp13=recomp_cycle1%temp(13)		 
         temp14=recomp_cycle1%temp(14)
         ...
         pres13=recomp_cycle1%pres(13)		 
         pres14=recomp_cycle1%pres(14)
		 
		 m_dot_turbine1=recomp_cycle1%m_dot_turbine
		 p_rhx_in1=recomp_cycle1%pres(12)
         p_mc1_out1=recomp_cycle1%pres(2)
        ...
        PHX=recomp_cycle1%PHX%Q_dot
        RHX=recomp_cycle1%RHX%Q_dot
        PC1=recomp_cycle1%PC1%Q_dot
        PC2=recomp_cycle1%PC2%Q_dot
