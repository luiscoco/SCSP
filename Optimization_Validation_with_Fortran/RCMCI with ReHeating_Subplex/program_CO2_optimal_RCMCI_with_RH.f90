...
!------------------------------------------------------------w
      ...
      subroutine carbondioxidesubplex( w_dot_net1, t_mc1_in1, t_mc2_in1, t_t_in1, p_mc1_in_guess1, &
	                      fixed_p_mc1_in1,  p_rhx_in_guess1, fixed_P_rhx_in1, t_rht_in1, &
                          ua_rec_total1, eta_mc1, eta_mc2, eta_rc1, eta_t1, eta_trh1, &
	                      n_sub_hxrs1, p_high_limit1, p_mc2_out_guess1, fixed_p_mc2_out1, &
					      pr_mc2_guess1, fixed_pr_mc2, recomp_frac_guess1, fixed_recomp_frac1, &
					      lt_frac_guess1, fixed_lt_frac1, tol1, opt_tol1, eta_thermal1, &
					      dp_lt1, dp_lt2, dp_ht1, dp_ht2, dp1_pc1, dp1_pc2, &
                          dp2_pc1, dp2_pc2, dp_phx1, dp_phx2, dp_rhx1, dp_rhx2, &
                          temp1, temp2, temp3, temp4, temp5, temp6, temp7, &
					      temp8, temp9, temp10, temp11, temp12, temp13, &
						  temp14, pres1, pres2, &
                          pres3, pres4, pres5, pres6, pres7, pres8, pres9, &
                          pres10,  pres11,  pres12,  pres13,  pres14,m_dot_turbine1, &
						  LT_mdoth,  LT_mdotc, &
                          LT_Tcin, LT_Thin, LT_Pcin, LT_Phin, LT_Pcout, LT_Phout, &
                          LT_Q, HT_mdoth, HT_mdotc, HT_Tcin, HT_Thin, HT_Pcin, HT_Phin, &
                          HT_Pcout, HT_Phout, HT_Q, LT_UA, HT_UA, LT_Effc, HT_Effc, &
						  PHX,RHX,PC1,PC2)
      ...
      integer :: i
      real(dp) , INTENT(INOUT):: w_dot_net1,t_mc1_in1,t_mc2_in1,t_t_in1,ua_rec_total1
      real(dp) , INTENT(INOUT):: p_high_limit1,p_mc2_out_guess1,pr_mc2_guess1, recomp_frac_guess1,lt_frac_guess1,tol1,opt_tol1
      real(dp) , INTENT(INOUT):: p_mc1_in_guess1,eta_thermal1,eta_mc1,eta_mc2,eta_rc1,eta_t1,eta_trh1
      real(dp) , INTENT(INOUT):: p_rhx_in_guess1,t_rht_in1,m_dot_turbine1,LT_UA,HT_UA,LT_Effc,HT_Effc	  
      real(dp) , INTENT(INOUT):: dp_lt1,dp_lt2,dp_ht1,dp_ht2,dp1_pc1,dp1_pc2,dp_phx1,dp_phx2,dp_rhx1,dp_rhx2
      real(dp) , INTENT(INOUT):: dp2_pc1,dp2_pc2
      ...
      logical , INTENT (INOUT):: fixed_p_mc2_out1,fixed_pr_mc2,fixed_recomp_frac1, fixed_lt_frac1,fixed_P_rhx_in1
      logical , INTENT (INOUT):: fixed_p_mc1_in1
      ...
      real(dp) , INTENT(INOUT):: PHX,RHX,PC1,PC2 
	  
! Define and optimize a design point.
      call optimal_design( &
           W_dot_net = w_dot_net1, &         ! power output of cycle (kW)
           T_mc1_in = t_mc1_in1, &             ! compressor inlet temperature (K)
           T_mc2_in = t_mc2_in1, &             ! compressor inlet temperature (K)
           T_t_in = t_t_in1, &               ! turbine inlet temperature (K)
           P_mc1_in_guess= p_mc1_in_guess1, &
           fixed_P_mc1_in= fixed_p_mc1_in1, &
           ...
           DP_PC1 = [-dp1_pc1,-dp2_pc1],  &  ! pressure drops in precooler (kPa if positive values)
           DP_PC2 = [-dp1_pc2,-dp2_pc2],  &  ! pressure drops in precooler (kPa if positive values)
           ...
           eta_mc1 = eta_mc1,              &  ! design-point efficiency of the main compressor; isentropic if positive, polytropic if negative
           eta_mc2 = eta_mc2,              &  ! design-point efficiency of the main compressor; isentropic if positive, polytropic if negative
           ...
           P_mc2_out_guess = p_mc2_out_guess1, &  ! compressor outlet temperature (kPa)
           fixed_P_mc2_out = fixed_p_mc2_out1,       &  ! do not vary P_mc_out
           PR_mc2_guess = pr_mc2_guess1,    &  ! initial guess for optimal pressure ratio
           fixed_PR_mc2 = fixed_pr_mc2,         &  ! vary PR to optimize efficiency
         ...
         p_mc2_out_guess1=recomp_cycle%pres(2)
		 pr_mc2_guess1=(recomp_cycle%pres(2))/(recomp_cycle%pres(14))
        ...
        PC1=recomp_cycle%PC1%Q_dot
        PC2=recomp_cycle%PC2%Q_dot
