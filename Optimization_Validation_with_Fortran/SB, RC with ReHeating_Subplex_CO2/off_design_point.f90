...
implicit none
private
public :: off_design, target_off_design,optimal_off_design
...
subroutine off_design( &
    ...
    T_mc_in,           &  ! [input] Main Compressor inlet temperature (K)
    T_t_in,            &  ! [input] Main Turbine inlet temperature (K)
    T_trh_in,          &  ! [input] ReHeating Turbine inlet temperature (K)
    P_trh_in,          &  ! [input] ReHeating Turbine inlet pressure (kPa)
    ...
    real(dp), intent(in) :: T_mc_in, T_t_in,T_trh_in,P_trh_in,P_mc_in, recomp_frac, N_mc, N_t, tol
    ...
    real(dp) :: m_dot_t, m_dot_mc, m_dot_rc, UA_LT, UA_HT, w_mc, w_rc, w_t, w_trh
    ...
    real(dp), dimension(2) :: DP_LT, DP_HT, DP_PC, DP_PHX, DP_RHX
    logical :: first_pass

    ! Parameters
    integer :: max_iter
    real(dp) :: temperature_tolerance  ! temperature differences below this are considered zero

    max_iter = 500
    temperature_tolerance = 1.0e-6_dp
    ...
    m_dot_loop: do m_dot_iter = 1, max_iter
        ...
        DP_RHX = hxr_pressure_drops(hxr=recomp_cycle%RHX, m_dots=[m_dot_t, 0.0_dp])
        ...
        ! Check if pressure are consistent values
        ! Pressure P6>P11>=P12>P7
    ...
    call CO2_TP(T=temp(11), P=pres(11), error_code=error_code, enth=enth(11), entr=entr(11), dens=dens(11))
    if (error_code /= 0) then
        error_trace%code = error_code
     ...
     call CO2_TP(T=temp(12), P=pres(12), error_code=error_code, enth=enth(12), entr=entr(12), dens=dens(12))
    if (error_code /= 0) then
        error_trace%code = error_code
        error_trace%lines(1) = 249
        error_trace%files(1) = 3
    ...
    w_t = enth(6) - enth(11)   ! specific work of turbine (kJ/kg) [positive]
    w_trh = enth(12) - enth(7)
...
subroutine target_off_design(  &
    ...
    ! Arguments
    type(RecompCycle), intent(inout) :: recomp_cycle
    real(dp), intent(in) :: T_mc_in, T_t_in,T_trh_in,P_trh_in, recomp_frac
    real(dp), intent(in) :: N_mc, N_t, target, lowest_pressure, highest_pressure, tol
    integer, intent(in)  :: target_code, N_sub_hxrs
    type(ErrorTrace), intent(out) :: error_trace

    ! Parameters
    integer, parameter :: max_iter = 100
    integer, parameter :: search_intervals = 1000  ! number of intervals to check for valid bounds before starting secant loop
    real(dp), parameter :: search_intervals1 = 1000
    ! Local Variables
    ...
    P_guesses_loop: do i = 0,search_intervals
    P_guesses(i)=(P_low + i * (P_high - P_low) / search_intervals1)
    end do P_guesses_loop
    ...
    biggest_cycle = recomp_cycle
        ...
        if (P_guess>=P_trh_in) exit  ! compressor inlet pressure is getting too big
        ...
        if (error_trace%code == 0) then
            if (recomp_cycle%pres(2) >= recomp_cycle%high_pressure_limit * 1.002_dp) exit  ! compressor inlet pressure is getting too big
...
subroutine optimal_off_design( &
    ...
    ! Arguments
    type(RecompCycle), intent(inout) :: recomp_cycle
    real(dp), intent(in) :: T_mc_in, T_t_in,T_trh_in, P_mc_in_guess
	real(dp), intent(in) :: P_trh_in_guess, recomp_frac_guess, N_mc_guess, N_t_guess, tol, opt_tol
    logical, intent(in)  :: fixed_P_mc_in, fixed_recomp_frac, fixed_N_mc, fixed_N_t,fixed_P_trh_in
    integer, intent(in)  :: value_code, N_sub_hxrs
    type(ErrorTrace), intent(out)  :: error_trace

    ! Subplex Parameters and Variables
    integer, parameter :: maxf = 200
    integer, parameter :: max_free_vars = 5
    ...
    index = 1
    !fixed_P_trh_in
    if (.not. fixed_P_trh_in) then
        x(index) = P_trh_in_guess
        scale(index) = 500.0_dp  ! P_mc_in scale (may need to be adjusted if optimal results are not satisfactory)
        index = index + 1
    end if 
    ...
    if (n > 0) then  ! need to call subplex
        ...
        call off_design(                     &
            ...
            T_mc_in = T_mc_in,               &
            T_t_in = T_t_in,                 &
            T_trh_in = T_trh_in,             &  ! [input] ReHeating Turbine inlet temperature (K)
            P_trh_in = P_trh_in_guess,       &  ! [input] ReHeating Turbine inlet pressure (kPa)
  ...
  !  )
    ...
    contains
        ...
        real(dp) function off_design_point_value(n, x)
            ...
            real(dp) :: P_trh_in_local, P_mc_in_local, recomp_frac_local, N_mc_local, N_t_local

            ! Extract input variables from x.
            index = 1
             if (.not. fixed_P_trh_in) then
                P_trh_in_local = x(index)
                index = index + 1
            else
                P_trh_in_local = P_trh_in_guess
            end if 
            if (.not. fixed_P_mc_in) then
            ...
            call off_design(                     &
                ...
                T_t_in = T_t_in,                 &
                T_trh_in = T_trh_in,             &  ! [input] ReHeating Turbine inlet temperature (K)
                P_trh_in = P_trh_in_local,       &  ! [input] ReHeating Turbine inlet pressure (kPa)
