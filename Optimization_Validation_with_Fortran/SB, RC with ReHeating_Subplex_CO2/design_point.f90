...
subroutine design( &
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in, P_mc_in, P_mc_out, P_rhx_in, T_rht_in, UA_LT, UA_HT, recomp_frac
    real(dp), intent(in) :: eta_mc, eta_rc, eta_t, eta_trh, tol
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC, DP_PHX, DP_RHX
    ...
    real(dp) :: w_mc, w_rc, w_t, w_trh, C_dot_min, Q_dot_max
    ...
    real(dp) :: m_dot_t, m_dot_mc, m_dot_rc, eta_mc_isen, eta_rc_isen, eta_t_isen, eta_trh_isen
    real(dp) :: min_DT_LT, min_DT_HT, UA_LT_calc, UA_HT_calc, Q_dot_LT, Q_dot_HT, UA_HT_residual, UA_LT_residual
    real(dp), dimension(12) :: temp, pres, enth, entr, dens
    ...
    T8_loop: do T8_iter = 1,max_iter
        ...
        T9_loop: do T9_iter = 1,max_iter
                ...
                m_dot_t = W_dot_net / (w_mc * (1.0_dp - recomp_frac) + w_rc * recomp_frac + w_t + w_trh)  ! required mass flow rate through turbine
...
subroutine optimal_design( &
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in, P_rhx_in_guess, T_rht_in, UA_rec_total, eta_mc, eta_rc, eta_t, eta_trh
    ...
    contains
        ...
        real(dp) function design_point_eta(n, x)
            ...
            real(dp) :: P_rhx_in_local, P_mc_in_local, P_mc_out_local, PR_mc_local, recomp_frac_local, LT_frac_local
            ...
            if (P_rhx_in_local < 100.0_dp) then  
                design_point_eta = 0.0_dp
                return
            end if
            ...
            print *,'ReHeating pressure inlet (bar):'
            print *,P_rhx_in_local
...
subroutine auto_optimal_design( &
    ...
    eta_rc,                     &  ! [input] design-point efficiency of the recompressor; isentropic if positive, polytropic if negative
    eta_t,                      &  ! [input] design-point efficiency of the turbine; isentropic if positive, polytropic if negative
    eta_trh,                    &  ! [input] design-point efficiency of the turbine; isentropic if positive, polytropic if negative
    N_sub_hxrs,                 &  ! [input] number of sub-heat exchangers to use when calculating UA value for a heat exchanger
    P_high_limit,               &  ! [input] maximum allowable pressure in cycle (kPa)
    tol,                        &  ! [input] cycle convergence tolerance
    opt_tol,                    &  ! [input] optimization convergence tolerance
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in, P_rhx_in, T_rht_in, UA_rec_total, eta_mc, eta_rc
	real(dp), intent(in) :: eta_t, eta_trh, P_high_limit, tol, opt_tol
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC, DP_PHX, DP_RHX
    type(ErrorTrace), intent(out)  :: error_trace
    type(RecompCycle), intent(out) :: recomp_cycle
