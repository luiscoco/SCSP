...
subroutine  turbine_rh_sizing(recomp_cycle, error_trace)
    ...
    recomp_cycle%t_rh%A_nozzle = recomp_cycle%m_dot_turbine / (C_s * D_out)  ! turbine effective nozzle area in m2
...
subroutine off_design_turbine_rh(turb_rh, T_in, P_in, P_out, N, error_trace, m_dot, T_out)
    ...
    if (turb_rh%nu < 1.0_dp) then
        eta_0 = 2.0_dp * turb_rh%nu * sqrt(1.0_dp - turb_rh%nu**2)  ! efficiency from Baines (1.0 at design point)
    else
        eta_0 = 0.0_dp  ! catches nu values just over 1, which leads to sqrt of negative number
    end if
    ...
    m_dot = C_s * turb_rh%A_nozzle * D_out  ! mass flow through turbine (kg/s)
