...
subroutine optimal_design( &
    ...
    contains
        ...
        real(dp) function design_point_eta(n, x)
            ...
            if (P_mc_in_local < 7400.0_dp) then
                design_point_eta = 0.0_dp
                return
            end if
...
subroutine auto_optimal_design( &
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in, P_rhx_in, T_rht_in, UA_rec_total, eta_mc, eta_rc, eta_t, eta_trh
	real(dp), intent(in) :: P_high_limit, tol, opt_tol
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC, DP_PHX, DP_RHX
    type(ErrorTrace), intent(out)  :: error_trace
    type(RecompCycle), intent(out) :: recomp_cycle
