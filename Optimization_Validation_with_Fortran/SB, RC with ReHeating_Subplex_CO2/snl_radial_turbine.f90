...
implicit none
private
public :: turbine_sizing, turbine_rh_sizing, off_design_turbine, off_design_turbine_rh
...
subroutine turbine_sizing(recomp_cycle, error_trace)
    ...
    P_out = recomp_cycle%pres(11)
    h_out = recomp_cycle%enth(11)
    D_out = recomp_cycle%dens(11)
...
subroutine turbine_rh_sizing(recomp_cycle, error_trace)
    ! Determine the turbine rotor diameter, effective nozzle area, and design-point shaft
    ! speed and store values in recomp_cycle%t.
    !
    ! Arguments:
    ...
    ! Check that a design-point shaft speed is available.
    if (recomp_cycle%t_rh%N_design <= 0.0_dp) then  ! link shafts
        recomp_cycle%t_rh%N_design = recomp_cycle%mc%N_design
        if (recomp_cycle%mc%N_design <= 0.0_dp) then
            error_trace%code = 7
            error_trace%lines(1) = 68
            error_trace%files(1) = 5
    ...
    ! Create references to cycle state properties for clarity.
    T_in = recomp_cycle%temp(12)
    D_in = recomp_cycle%dens(12)
    h_in = recomp_cycle%enth(12)
    s_in = recomp_cycle%entr(12)
    ...
    ! Determine necessary turbine parameters.
    recomp_cycle%t_rh%nu = nu_design
    w_i = h_in - h_s_out  ! isentropic specific work of turbine (kJ/kg)
    C_s = sqrt(2.0_dp * w_i * 1000.0_dp)  ! spouting velocity in m/s
    U_tip = recomp_cycle%t_rh%nu * C_s  ! rearrange definition of nu
    recomp_cycle%t_rh%D_rotor = U_tip / (0.5_dp * recomp_cycle%t_rh%N_design * 0.104719755_dp)  ! turbine diameter in m
    recomp_cycle%t_rh%A_nozzle = recomp_cycle%m_dot_turbine / (C_s * D_in)  ! turbine effective nozzle area in m2

    ! Set other turbine variables.
    recomp_cycle%t_rh%w_tip_ratio = U_tip / ssnd_in  ! ratio of the tip speed to local (turbine inlet) speed of sound
    recomp_cycle%t_rh%eta_design = (h_in - h_out) / w_i  ! definition of isentropic efficiency
    recomp_cycle%t_rh%eta = recomp_cycle%t_rh%eta_design
    recomp_cycle%t_rh%N = recomp_cycle%t_rh%N_design

end subroutine turbine_rh_sizing
...
subroutine off_design_turbine_rh(turb_rh, T_in, P_in, P_out, N, error_trace, m_dot, T_out)
    ! Solve for the outlet state of 'turb' given its inlet conditions, outlet pressure, and shaft speed.
    !
    ! Inputs:
    !   turb -- a Turbine object, with design-point values and sizing set
    ...
    ! Arguments
    type(Turbine), intent(inout) :: turb_rh
    real(dp), intent(in) :: T_in, P_in, P_out, N
    type(ErrorTrace), intent(out) :: error_trace
    real(dp), intent(out) :: m_dot, T_out
    ...
    ! Apply the radial turbine equations for efficiency.
    C_s = sqrt(2.0_dp * (h_in - h_s_out) * 1000.0_dp)  ! spouting velocity (m/s)
    U_tip = turb_rh%D_rotor * 0.5_dp * N * 0.104719755_dp  ! turbine tip speed (m/s)
    turb_rh%nu = U_tip / C_s  ! ratio of tip speed to spouting velocity
    
    !eta_0 = 0.179921180_dp + 1.3567_dp*turb%nu + 1.3668_dp*turb%nu**2 - 3.0874_dp*turb%nu**3 + 1.0626_dp*turb%nu**4
    eta_0 = (((1.0626_dp * turb_rh%nu - 3.0874_dp) * turb_rh%nu + 1.3668_dp) * turb_rh%nu + 1.3567_dp) * turb_rh%nu + 0.179921180_dp
    eta_0 = max(eta_0, 0.0_dp)
    eta_0 = min(eta_0, 1.0_dp)
    turb_rh%eta = eta_0 * turb_rh%eta_design  ! actual turbine efficiency
    
    ! Calculate the outlet state and allowable mass flow rate.
    h_out = h_in - turb_rh%eta * (h_in - h_s_out)  ! enthalpy at turbine outlet
    call CO2_PH(P=P_out, H=h_out, error_code=error_code, temp=T_out, dens=D_out)
    if (error_code /= 0) then
        error_trace%code = error_code
        error_trace%lines(1) = 174
    ...
    m_dot = C_s * turb_rh%A_nozzle * D_in  ! mass flow through turbine (kg/s)
    turb_rh%w_tip_ratio = U_tip / ssnd_in  ! ratio of the tip speed to the local (turbine inlet) speed of sound
    turb_rh%N = N

end subroutine off_design_turbine_rh

end module snl_turbines