...
subroutine design( &
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in, P_mc_in, P_mc_out, P_rhx1_in, T_rht1_in, P_rhx2_in, T_rht2_in, UA_LT, UA_HT, recomp_frac
    real(dp), intent(in) :: eta_mc, eta_rc, eta_t, eta_trh1, eta_trh2, tol
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC, DP_PHX, DP_RHX2, DP_RHX2 
    ...
    real(dp) :: w_mc, w_rc, w_t, w_trh1, w_trh2, C_dot_min, Q_dot_max
    ...
    real(dp) :: m_dot_t, m_dot_mc, m_dot_rc, eta_mc_isen, eta_rc_isen, eta_t_isen, eta_trh1_isen, eta_trh2_isen
	...
	if (DP_RHX1(1) < 0.0_dp) then
        pres(12) = pres(11) - pres(11) * abs(DP_RHX1(1))  ! relative pressure drop specified for RHX1
    else
        pres(12) = pres(11) - DP_RHX1(1)                 ! absolute pressure drop specified for RHX1
    end if
	
	if (DP_RHX2(1) < 0.0_dp) then
        pres(14) = pres(13) - pres(11) * abs(DP_RHX2(1))  ! relative pressure drop specified for RHX2
    else
        pres(14) = pres(13) - DP_RHX2(1)                 ! absolute pressure drop specified for RHX2
    end if
    ...
    if (eta_trh1 < 0.0_dp) then
        call isen_eta_from_poly_eta(   &
            T_in = temp(12),            &
            P_in = pres(12),            &
            P_out = pres(13),           &
            poly_eta = abs(eta_trh1),     &
            is_comp = .false.,         &
            error_trace = error_trace, &
            isen_eta = eta_trh1_isen      &
            )
        if (error_trace%code /= 0) then
            index = next_trace_index(error_trace)
            error_trace%lines(index) = 173
        ...
        eta_trh1_isen = eta_trh1
    end if
	
	if (eta_trh2 < 0.0_dp) then
        call isen_eta_from_poly_eta(   &
            T_in = temp(14),            &
            P_in = pres(14),            &
            P_out = pres(7),           &
            poly_eta = abs(eta_trh2),     &
            is_comp = .false.,         &
            error_trace = error_trace, &
            isen_eta = eta_trh2_isen      &
            )
        if (error_trace%code /= 0) then
            index = next_trace_index(error_trace)
            error_trace%lines(index) = 173
  ...
  !-----------------------------------------------------------------------------------------------------------------------
    ...
    call calculate_turbomachine_outlet( &  ! ReHeating1 Turbine
        T_in = temp(12),                 &
        P_in = pres(12),                 &
        P_out = pres(13),                &
        eta = eta_trh1_isen,               &
        ...
        !print *,'Turbine  Densidad out:'
		!print *, dens(13)	
		!print *,'ReHeating Turbine  Specific Work:'
		!print *, w_trh1 
		
	 call calculate_turbomachine_outlet( &  ! ReHeating2 Turbine
        T_in = temp(14),                 &
        P_in = pres(14),                 &
        P_out = pres(14),                &
        eta = eta_trh1_isen,               &
    ...
    T8_loop: do T8_iter = 1,max_iter
        ...
        T9_loop: do T9_iter = 1,max_iter
                ...
                m_dot_t = W_dot_net / (w_mc * (1.0_dp - recomp_frac) + w_rc * recomp_frac + w_t + w_trh1 +  w_trh2)  ! required mass flow rate through turbine
	...
	recomp_cycle%RHX1%Q_dot = m_dot_t * (enth(12) - enth(11))
    recomp_cycle%RHX1%DP_design = [pres(11) - pres(12), 0.0_dp]
    recomp_cycle%RHX1%m_dot_design = [m_dot_t, 0.0_dp]
	
	recomp_cycle%RHX2%Q_dot = m_dot_t * (enth(14) - enth(13))
    recomp_cycle%RHX2%DP_design = [pres(13) - pres(14), 0.0_dp]
    recomp_cycle%RHX2%m_dot_design = [m_dot_t, 0.0_dp]
    ...
    recomp_cycle%W_dot_net = w_mc * m_dot_mc + w_rc * m_dot_rc + w_t * m_dot_t + w_trh1 * m_dot_t + w_trh2 * m_dot_t
	
	!print *,'Potencia del ciclo:'
	!print *, w_mc * m_dot_mc + w_rc * m_dot_rc + w_t * m_dot_t + w_trh * m_dot_t
	...
	!print *,'Calor absorbido por el condensador del ciclo:'
...
subroutine optimal_design( &  !
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in, P_rhx1_in_guess, T_rht1_in, P_rhx2_in_guess, T_rht2_in, UA_rec_total, eta_mc, eta_rc, eta_t, eta_trh1, eta_trh2
    real(dp), intent(in) :: P_high_limit, P_mc_out_guess, PR_mc_guess, recomp_frac_guess, LT_frac_guess, tol, opt_tol
    logical, intent(in)  :: fixed_P_mc_out, fixed_PR_mc, fixed_recomp_frac, fixed_LT_frac, fixed_P_rhx_in
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC, DP_PHX, DP_RHX1, DP_RHX2
    type(ErrorTrace), intent(out)  :: error_trace
    type(RecompCycle), intent(out) :: recomp_cycle
    ...
    integer  :: iflag, iwork(150), nfe
    real(dp) :: subplex_fmin, scale(max_free_vars), work(150), x(max_free_vars)
    ...
    x = 0.0_dp
    index = 1
    if (.not. fixed_P_rhx1_in) then
        x(index) = P_rhx1_in_guess
        scale(index) = 50.0_dp  ! pressure scale reheating1 turbine
        index = index + 1
    end if
	if (.not. fixed_P_rhx2_in) then
        x(index) = P_rhx2_in_guess
        scale(index) = 50.0_dp  ! pressure scale reheating2 turbine
        index = index + 1
    end if
    if (.not. fixed_P_mc_out) then
        x(index) = P_mc_out_guess
        scale(index) = 50.0_dp  ! pressure scale main compressor
        index = index + 1
    end if
    if (.not. fixed_PR_mc) then
        x(index) = PR_mc_guess
        scale(index) = 0.02_dp  ! pressure ratio scale main compressor
        index = index + 1
    end if    
    ...
    if (n > 0) then  ! call subplex
        ...
        call design(                                         & !
            ...
            P_rhx1_in = P_rhx1_in_guess,                     & !
			P_rhx2_in = P_rhx2_in_guess,                     & ! 
            T_rht1_in = T_rht1_in,                           & !
            DP_LT = DP_LT,                                   & !
            DP_HT = DP_HT,                                   & ! 
            DP_PC = DP_PC,                                   & !
    ...
    contains
        ...
        real(dp) function design_point_eta(n, x)
            ...
            real(dp) :: P_rhx1_in_local, P_rhx2_in_local, P_mc_in_local, P_mc_out_local, PR_mc_local, recomp_frac_local, LT_frac_local

            ! Extract input variables from x.
            index = 1
			if (.not. fixed_P_rhx1_in) then
                P_rhx1_in_local = x(index)
                index = index + 1
            else
                P_rhx1_in_local = P_rhx1_in_guess
            end if
			if (.not. fixed_P_rhx2_in) then
                P_rhx2_in_local = x(index)
                index = index + 1
            else
                P_rhx2_in_local = P_rhx2_in_guess
            end if
            if (.not. fixed_P_mc_out) then
            ...
            if (P_mc_in_local < 7377.0_dp) then
                design_point_eta = 0.0_dp
                return
            end if
            if (P_rhx1_in_local < 100.0_dp) then  
                design_point_eta = 0.0_dp
                return
            end if
			 if (P_rhx2_in_local < 100.0_dp) then  
                design_point_eta = 0.0_dp
                return
            end if
            ...
            print *,'ReHeating1 pressure inlet (bar):'
            print *,P_rhx1_in_local
			
			  print *,'ReHeating2 pressure inlet (bar):'
            print *,P_rhx2_in_local
			
            print *,'Compressor outlet pressure (bar):'
            print *,P_mc_in_local
            ...
            call design(                                         & !
                ...
                P_rhx1_in = P_rhx1_in_local,                     & !
				T_rht1_in = T_rht1_in,                           & !
				P_rhx2_in = P_rhx2_in_local,                     & !
				T_rht2_in = T_rht2_in,                           & !
				DP_LT = DP_LT,                                   & !
                DP_HT = DP_HT,                                   & !
                DP_PC = DP_PC,                                   & !
...
subroutine auto_optimal_design( &  !
    ...
    eta_rc,                     &  ! [input] design-point efficiency of the recompressor; isentropic if positive, polytropic if negative
    eta_t,                      &  ! [input] design-point efficiency of the turbine; isentropic if positive, polytropic if negative
    eta_trh1,                   &  ! [input] design-point efficiency of the turbine; isentropic if positive, polytropic if negative
	eta_trh2,                   &  ! [input] design-point efficiency of the turbine; isentropic if positive, polytropic if negative
    N_sub_hxrs,                 &  ! [input] number of sub-heat exchangers to use when calculating UA value for a heat exchanger
    P_high_limit,               &  ! [input] maximum allowable pressure in cycle (kPa)
    tol,                        &  ! [input] cycle convergence tolerance
    opt_tol,                    &  ! [input] optimization convergence tolerance
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in, P_rhx1_in, T_rht1_in, P_rhx2_in, T_rht2_in, UA_rec_total, eta_mc, eta_rc
	real(dp), intent(in) :: eta_t, eta_trh1, eta_trh2, P_high_limit, tol, opt_tol
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC, DP_PHX, DP_RHX1, DP_RHX2
    type(ErrorTrace), intent(out)  :: error_trace
    type(RecompCycle), intent(out) :: recomp_cycle
    ...
    call optimal_design(                 &  ! recompression cycle
        ...
        T_mc_in = T_mc_in,               &  !
        T_t_in = T_t_in,                 &  !
        P_rhx1_in_guess = 11000.0_dp,    &  !
        fixed_P_rhx1_in = .false.,       &  !
        T_rht2_in = T_rht2_in,           &  !
		P_rhx2_in_guess = 11000.0_dp,    &  !
        fixed_P_rhx2_in = .false.,       &  !
        T_rht_in = T_rht_in,             &  !
		DP_LT = DP_LT,                   &  !
        DP_HT = DP_HT,                   &  !
        DP_PC = DP_PC,                   &  !
