...
subroutine design( &
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in, P_mc_in, P_mc_out, P_rhx_in, T_rht_in, P_pc_in, T_pc_in, P_pc_out
	real(dp), intent(in) :: UA_LT, UA_HT, recomp_frac
    real(dp), intent(in) :: eta_mc, eta_rc, eta_pc, eta_t, eta_trh, tol
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC, DP_PHX, DP_RHX, DP_COOLER
    ...
    real(dp) :: w_mc, w_rc, w_pc, w_t, w_trh, C_dot_min, Q_dot_max
    ...
    real(dp) :: m_dot_t, m_dot_mc, m_dot_rc, m_dot_pc, eta_mc_isen, eta_rc_isen,eta_pc_isen, eta_t_isen, eta_trh_isen
	...
	if (DP_RHX(1) < 0.0_dp) then
        pres(12) = pres(11) - pres(11) * abs(DP_RHX(1))  ! relative pressure drop specified for RHX
    else
        pres(12) = pres(11) - DP_RHX(1)                 ! absolute pressure drop specified for RHX
    end if
    
    if (DP_COOLER(2) < 0.0_dp) then
        pres(9) = pres(13) / (1.0_dp - abs(DP_COOLER(2)))  ! relative pressure drop specified for precooler: P1=P9-P9*rel_DP => P1=P9*(1-rel_DP)
    else
        pres(9) = pres(13) + DP_COOLER(2)                  ! absolute pressure drop specified for precooler
    end if	
	
	if (DP_PC(2) < 0.0_dp) then
        pres(14) = pres(1) / (1.0_dp - abs(DP_PC(2)))  ! relative pressure drop specified for precooler: P1=P9-P9*rel_DP => P1=P9*(1-rel_DP)
    else
        pres(14) = pres(1) + DP_PC(2)                  ! absolute pressure drop specified for precooler
    end if
	...
	!print *, 'Precompressor Efficiency LUIS COCO PRUEBA:'
	!print *, eta_pc 
		
	if (eta_pc < 0.0_dp) then
        call isen_eta_from_poly_eta(   &
            T_in = temp(13),            &
            P_in = pres(13),            &
            P_out = pres(14),           &
    ...
    if (eta_t < 0.0_dp) then
		...
		!print *, 'Luis Coco Primer3 Escalon:'
    else
        eta_t_isen = eta_t
    end if
    ...
    if (eta_trh < 0.0_dp) then
        call isen_eta_from_poly_eta(   &
            T_in = temp(12),            &
            P_in = pres(12),            &
            P_out = pres(7),           &
            poly_eta = abs(eta_trh),     &
            is_comp = .false.,         &
            error_trace = error_trace, &
            isen_eta = eta_trh_isen      &
            )
        if (error_trace%code /= 0) then
            index = next_trace_index(error_trace)
            error_trace%lines(index) = 173
		...
		!print *, 'Luis Coco Seguno2 Escalon:'
    else
    ...
    call calculate_turbomachine_outlet( &  ! Calculate Pre-Compressor Outlet and Specifica Work
        T_in = temp(13),                 &
        P_in = pres(13),                 &
        P_out = pres(14),                &
        eta = eta_pc_isen,              &
        ...
        !print *, 'Luis Coco Segundo1 Escalon:'
        !print *, 'Pre-Compressor:'
        !print *, 'Pre-Compressor Temperature in:'
        !print *, temp(13)	
        !print *, 'Pre-CompressorPressure in:'
		!print *, pres(13) 
		!print *, 'Pre-Compressor Entalpia in:'
		!print *, enth(13)
		!print *,'Pre-CompressorEntropia in:'
		!print *, entr(13)
		!print *,'Pre-Compressor Densidad in:'
		!print *, dens(13)
		!print *,'Pre-Compressor Pressure out:'
		!print *, pres(14)	
		!print *,'Pre-Compressor Ental out:'
		!print *, enth(14)	
		!print *,'Pre-Compressor Entropia out:'
		!print *, entr(14)
		!print *,'Pre-Compressor Temperature out:'
		!print *, temp(14) 
        !print *,'Pre-Compressor Densidad out:'
		!print *, dens(14)	
		!print *,'Pre-CompressorEfficiency:'
		!print *, eta_pc_isen
		!print *,'Pre-Compressor Specific Work:'
		!print *, w_pc 
    ...
    if (error_trace%code /= 0) then
        ...
        error_trace%files(index) = 2
		print *, 'Error en Precompressor Luis Coco:'
        return
    end if
	...
	call calculate_turbomachine_outlet( &  ! Main Compressor
        ...
        !print *,'Main Compressor Temperature in:'
		!print *, temp(1)	
		!print *,'Main Compressor Pressure in:'
		!print *, pres(1) 
        !print *,'Main Compressor Pressure out:'
		!print *, pres(2)	
		!print *,'Main Compressor Efficiency:'
		!print *, eta_mc_isen
		!print *,'Main Compressor Entalpia in:'
		!print *, enth(1)
		!print *,'Main Compressor Entropia in:'
		!print *, entr(1)
		!print *,'Main Compressor Densidad in:'
		!print *, dens(1)
		!print *,'Main Compressor Ental out:'
		!print *, enth(2)	
		!print *,'Main Compressor Entropia out:'
		!print *, entr(2)
		!print *,'Main Compressor Temperature out:'
		!print *, temp(2) 
        !print *,'Main Compressor Densidad out:'
		!print *, dens(2)	
		!print *,'Main Compressor Specific Work:'
		!print *, w_mc 
    ...
    if (error_trace%code /= 0) then
        ...
        error_trace%lines(index) = 193
        error_trace%files(index) = 2
		!print *, 'Error en Main Compressor Luis Coco:'
        return
    end if
	
    call calculate_turbomachine_outlet( &  ! Main Turbine
        T_in = temp(6),                 &
        P_in = pres(6),                 &
        P_out = pres(11),                &
        eta = eta_t_isen,               &
    ...
    call calculate_turbomachine_outlet( &  ! Reheating Turbine
        T_in = temp(12),                 &
        P_in = pres(12),                 &
        P_out = pres(7),                &
        eta = eta_t_isen,               &
		...
		!print *, 'Luis Coco Segundo4 Escalon:'
    ...
    if (error_trace%code /= 0) then
        ...
        error_trace%files(index) = 2
		print *, 'Luis Coco Segundo5 Escalon:'
        return
    end if
    ...
    if (recomp_frac >= 1.0d-12) then
        if (eta_rc < 0.0_dp) then  ! need to convert polytropic efficiency to isentropic efficiency
            call isen_eta_from_poly_eta(   &
                T_in = temp(2),            &  ! lowest possible recompressor work occurs when temp(14) == temp(2)
                P_in = pres(14),            &
                P_out = pres(10),          &
                poly_eta = abs(eta_rc),    &
                is_comp = .true.,          &
	...
	call calculate_turbomachine_outlet( &  ! ReCompressor
		...
		!print *, pres(14) 
		!print *,'ReCompressor Temperature out:'
		!print *, temp(10)
	...
	!print *, 'Reheating Turbine:'
	!print *, 'Presion 12:'
	!print *, pres(12)
	!print *, 'Temperatura 12:'
	!print *, temp(12)
	!print *, 'Presion 7:'
	!print *, pres(7)
	!print *, 'Temperatura 7:'
    ...
    T8_loop: do T8_iter = 1,max_iter
        ...
        T9_loop: do T9_iter = 1,max_iter
                ...
                if (recomp_frac >= 1.0e-12_dp) then
                    ...
                    call CO2_TP(T=temp(14), P=pres(14), error_code=error_code, enth=enth(14), entr=entr(14), dens=dens(14))  ! fully define state 9
                    if (error_code /= 0) then
                        error_trace%code = error_code
                    ...
                    temp(10) = temp(14)  ! assume state 10 is the same as state 9
                    enth(10) = enth(14)
                    entr(10) = entr(14)
                    dens(10) = dens(14)
                end if
                ...
                m_dot_t = W_dot_net / (w_pc + w_mc * (1.0_dp - recomp_frac) + w_rc * recomp_frac + w_t + w_trh)  ! required mass flow rate through turbine
                
				!print*, 'Caudal del Ciclo:'
				!print *, m_dot_t
...
!-------------------------------------------------------------------------------------------------------------------------				
    ...
    ! Set relevant values for other heat exchangers (PHX, RHX, PC, COOLER).
	...
	recomp_cycle%RHX%Q_dot = m_dot_t * (enth(12) - enth(11))
    recomp_cycle%RHX%DP_design = [pres(11) - pres(12), 0.0_dp]
    recomp_cycle%RHX%m_dot_design = [m_dot_t, 0.0_dp]
    
	recomp_cycle%PC%Q_dot = m_dot_t * (enth(13) - enth(9))
    recomp_cycle%PC%DP_design = [0.0_dp, pres(13) - pres(9)]
    recomp_cycle%PC%m_dot_design = [0.0_dp, m_dot_mc]
	
	recomp_cycle%COOLER%Q_dot = m_dot_mc * (enth(1) - enth(14))
    recomp_cycle%COOLER%DP_design = [0.0_dp, pres(1) - pres(14)]
    recomp_cycle%COOLER%m_dot_design = [0.0_dp, m_dot_mc]
    
	! Calculate cycle performance metrics.
    recomp_cycle%recomp_frac = recomp_frac
	
    recomp_cycle%W_dot_net = w_pc * m_dot_t + w_mc * m_dot_mc + w_rc * m_dot_rc + w_t * m_dot_t + w_trh * m_dot_t
	
    !print *,'Potencia del ciclo:'
    !print *, w_pc * m_dot_t + w_mc * m_dot_mc + w_rc * m_dot_rc + w_t * m_dot_t + w_trh * m_dot_t
	
    !print *,'Calor aportado al ciclo del ciclo:'
	...
	!print *,recomp_cycle%COOLER%Q_dot
	
	!print *,'Calor absorbido por el Segundo Condensador del ciclo:'
	!print *,recomp_cycle%PC%Q_dot
...
subroutine optimal_design( &
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in, P_rhx_in_guess, T_rht_in,P_pc_in_guess,T_pc_in
    real(dp), intent(in) :: UA_rec_total, eta_mc, eta_rc, eta_pc, eta_t, eta_trh
	real(dp), intent(in) :: P_high_limit, P_mc_out_guess, PR_mc_guess, recomp_frac_guess, LT_frac_guess, tol, opt_tol
    logical, intent(in)  :: fixed_PR_mc,fixed_P_rhx_in,fixed_P_mc_out, fixed_recomp_frac, fixed_LT_frac,fixed_P_pc_in
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC, DP_PHX, DP_RHX, DP_COOLER
    type(ErrorTrace), intent(out)  :: error_trace
    type(RecompCycle), intent(out) :: recomp_cycle
    ...
    integer, parameter :: max_free_vars = 7
    integer, parameter :: mode = 0
    integer  :: iflag, iwork(300), nfe
    real(dp) :: subplex_fmin, scale(max_free_vars), work(300), x(max_free_vars)
    ...
    x = 0.0_dp
    index = 1
    if (.not. fixed_P_pc_in) then
        x(index) = P_pc_in_guess
        scale(index) = 50.0_dp  ! pressure scale
        index = index + 1
    end if
    if (.not. fixed_P_rhx_in) then
        x(index) = P_rhx_in_guess
        scale(index) = 50.0_dp  ! pressure scale
        index = index + 1
    end if
    ...
    if (n > 0) then  ! call subplex
        ...
        call design(                                         &
            ...
            P_rhx_in = P_rhx_in_guess,                       &     
            T_rht_in = T_rht_in,                             &
            P_pc_in = P_pc_in_guess,                         &
            T_pc_in = T_pc_in,                               &
            P_pc_out = P_mc_out_guess / PR_mc_guess,         &     
            DP_LT = DP_LT,                                   &
            DP_HT = DP_HT,                                   &
            DP_PC = DP_PC,                                   &
            DP_PHX = DP_PHX,                                 &
    ...
    contains
        ...
        real(dp) function design_point_eta(n, x)
            ...
            real(dp) :: P_pc_in_local, P_mc_in_local, P_mc_out_local, PR_mc_local, recomp_frac_local, LT_frac_local, P_rhx_in_local

            ! Extract input variables from x.
            index = 1
            if (.not. fixed_P_pc_in) then
                P_pc_in_local = x(index)
                index = index + 1
            else
                P_pc_in_local = P_pc_in_guess
            end if
			
			if (.not. fixed_P_rhx_in) then
                P_rhx_in_local = x(index)
                index = index + 1
            else
                P_rhx_in_local = P_rhx_in_guess
            end if
            ...
            if (P_rhx_in_local < 100.0_dp) then  ! low-pressure limit
                design_point_eta = 0.0_dp
                return
            end if
            ...
            if (P_pc_in_local < 7377.0_dp) then !pre-compressor inlet pressure limit
                design_point_eta = 0.0_dp
                return
            end if
            ...
            call design(                                         &
                ...
                P_rhx_in = P_rhx_in_local,                       &
                T_rht_in = T_rht_in,                             &
                P_pc_in = P_pc_in_local,                         &
                T_pc_in = T_pc_in,                               &
                P_pc_out = P_mc_in_local,                        &
                DP_LT = DP_LT,                                   &
                DP_HT = DP_HT,                                   &
                DP_PC = DP_PC,                                   &
                DP_PHX = DP_PHX,                                 &
...
subroutine auto_optimal_design( &
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in, P_rhx_in,T_rht_in, P_pc_in, T_pc_in, UA_rec_total
	real(dp), intent(in) :: eta_mc, eta_rc, eta_pc, eta_t, eta_trh, P_high_limit, tol, opt_tol
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC, DP_PHX, DP_RHX, DP_COOLER
    type(ErrorTrace), intent(out)  :: error_trace
    type(RecompCycle), intent(out) :: recomp_cycle
