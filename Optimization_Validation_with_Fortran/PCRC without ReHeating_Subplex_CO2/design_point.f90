...
subroutine design( &
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in, P_mc_in, P_mc_out,  P_pc_in, T_pc_in, P_pc_out
	real(dp), intent(in) :: UA_LT, UA_HT, recomp_frac
    real(dp), intent(in) :: eta_mc, eta_rc, eta_pc, eta_t, tol
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC, DP_PHX, DP_COOLER
    ...
    real(dp) :: w_mc, w_rc, w_pc, w_t, C_dot_min, Q_dot_max
    ...
    real(dp) :: m_dot_t, m_dot_mc, m_dot_rc, m_dot_pc, eta_mc_isen, eta_rc_isen,eta_pc_isen, eta_t_isen
    ...
    T8_loop: do T8_iter = 1,max_iter
        ...
        T9_loop: do T9_iter = 1,max_iter
                ...
                m_dot_t = W_dot_net / (w_pc + w_mc * (1.0_dp - recomp_frac) + w_rc * recomp_frac + w_t )  ! required mass flow rate through turbine
                !print *, 'w_pc:'
	            !print *, w_pc
                !print *, 'w_mc:'
	            !print *, w_mc
...
!-------------------------------------------------------------------------------------------------------------------------				
    ...
    recomp_cycle%W_dot_net = w_pc * m_dot_t + w_mc * m_dot_mc + w_rc * m_dot_rc + w_t * m_dot_t 
...
subroutine optimal_design( &
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in,P_pc_in_guess,T_pc_in
    real(dp), intent(in) :: UA_rec_total, eta_mc, eta_rc, eta_pc, eta_t
	real(dp), intent(in) :: P_high_limit, P_mc_out_guess, PR_mc_guess, recomp_frac_guess, LT_frac_guess, tol, opt_tol
    logical, intent(in)  :: fixed_PR_mc,fixed_P_mc_out, fixed_recomp_frac, fixed_LT_frac,fixed_P_pc_in
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC, DP_PHX, DP_COOLER
    type(ErrorTrace), intent(out)  :: error_trace
    type(RecompCycle), intent(out) :: recomp_cycle
    ...
    contains
        ...
        real(dp) function design_point_eta(n, x)
            ...
            real(dp) :: P_pc_in_local, P_mc_in_local, P_mc_out_local, PR_mc_local, recomp_frac_local, LT_frac_local
...
subroutine auto_optimal_design( &
    ...
    ! Arguments
    real(dp), intent(in) :: W_dot_net, T_mc_in, T_t_in,P_pc_in, T_pc_in
	real(dp), intent(in) :: UA_rec_total, eta_mc, eta_rc, eta_pc, eta_t, P_high_limit, tol, opt_tol
    integer, intent(in)  :: N_sub_hxrs
    real(dp), dimension(2), intent(in) :: DP_LT, DP_HT, DP_PC, DP_PHX,DP_COOLER
    type(ErrorTrace), intent(out)  :: error_trace
    type(RecompCycle), intent(out) :: recomp_cycle
