...
implicit none
private
public :: precompressor_sizing,compressor_sizing, recompressor_sizing, off_design_precompressor, off_design_compressor, off_design_recompressor
...
subroutine precompressor_sizing(recomp_cycle, error_trace)
    ...
    D_in = recomp_cycle%dens(13)
    h_in = recomp_cycle%enth(13)
    s_in = recomp_cycle%entr(13)
    T_out = recomp_cycle%temp(14)
    P_out = recomp_cycle%pres(14)
    h_out = recomp_cycle%enth(14)
    D_out = recomp_cycle%dens(14)
    ...
    m_dot = recomp_cycle%m_dot_turbine   ! mass flow rate through compressor (kg/s)
    ...
    recomp_cycle%pc1%D_rotor = sqrt(m_dot / (snl_phi_design * D_in * U_tip))  ! rearranging definition of flow coefficient
    N_rad_s = U_tip * 2.0_dp / recomp_cycle%pc1%D_rotor   ! shaft speed in rad/s
    recomp_cycle%pc1%N_design = N_rad_s * 9.549296590_dp  ! shaft speed in rpm

    ! Set other compressor variables.
    recomp_cycle%pc1%w_tip_ratio = U_tip / ssnd_out     ! ratio of the tip speed to local (comp outlet) speed of sound
    recomp_cycle%pc1%eta_design = w_i / (h_out - h_in)  ! definition of isentropic efficiency
    recomp_cycle%pc1%eta = recomp_cycle%pc1%eta_design
    recomp_cycle%pc1%phi_design = snl_phi_design
    recomp_cycle%pc1%phi = snl_phi_design
    recomp_cycle%pc1%phi_min = snl_phi_min
    recomp_cycle%pc1%phi_max = snl_phi_max
    recomp_cycle%pc1%N = recomp_cycle%pc1%N_design
    recomp_cycle%pc1%surge = .false.

end subroutine precompressor_sizing
...
subroutine recompressor_sizing(recomp_cycle, error_trace)
    ...
    ! Create references to cycle state properties for clarity.
    D_in = recomp_cycle%dens(14)
    h_in = recomp_cycle%enth(14)
    s_in = recomp_cycle%entr(14)
...
subroutine off_design_precompressor(comp, T_in, P_in, m_dot, N, error_trace, T_out, P_out)
    ...
    ! Check that the specified mass flow rate is possible with the precompressor's current shaft speed.
    ...
    ! Calculate the precompressor outlet state.
...
end subroutine off_design_precompressor
