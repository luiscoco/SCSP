...
namespace COBYLA_Validation
{
    public class CobylaTests
    {
        private const double rhobeg = 0.5;
        private const double rhoend1 = 0.001;
        private const double rhoend2 = 1.0e-8;
        private const int maxfun = 2000;

        //Tuple<"Validation Case Title", CalcfcDelegate , N, M, VARIALBLES INITIAL VALUES ARRAY, double>
        ...
        //public Tuple<string, CalcfcDelegate, int, int, double[], double> testCase1 = Tuple.Create("1 (Simple quadratic)", (CalcfcDelegate)calcfc1, 2, 0, new[] { -1.0, 0.0 }, 1.0e-5);

        //public Tuple<string, CalcfcDelegate, int, int, double[], double> testCase9 = Tuple.Create("9 (Hock and Schittkowski 100)", (CalcfcDelegate)calcfc9, 7, 4,
        //                         new[] { 2.330499, 1.951372, -0.4775414, 4.365726, -0.624487, 1.038131, 1.594227 }, 1.0e-5);

        //public Tuple<string, CalcfcDelegate, int, int, double[], double> testCase10 = Tuple.Create("10 (Hexagon area)", (CalcfcDelegate)calcfc10, 9, 14,
        //                         new[] { 0.688341, 0.725387, -0.284033, 0.958814, 0.688341, 0.725387, -0.284033, 0.958814, 0.0 },
        //                         1.0e-5);
        ...
        public double RunTestProblem(CalcfcDelegate calcfc, int n, int m, double rhoend, double[] xopt)
        {
            ...
            //Assert.That(result.Status,Is.EqualTo(OptimizationStatus.Normal).Or.EqualTo(OptimizationStatus.MAXFUN_Reached));
    ...
    class Program
    {
        ...
        {
            CobylaTests prueba1 = new CobylaTests();
            foreach (Tuple<string, CalcfcDelegate, int, int, double[], double> x in prueba1.TestCases)
            {
                prueba1.TestProblem(x);
