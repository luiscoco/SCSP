...
Private Sub drawChart(viewer As WebChartViewer)
    ...
    Dim downloadFormat As String = Request("download")
    If Not string.IsNullOrEmpty(downloadFormat) Then
        Dim fname As String = "demo_" & c.formatValue(timeStamps(UBound(timeStamps)), _
            "yyyymmddhhnnss")
        If downloadFormat = "pdf" Then
            ' Out in PDF and stream as attachment
            viewer.Image = c.makeWebImage(Chart.PDF)
            viewer.StreamChart(fname & ".pdf", True)
        Else
            ' Out in PNG and stream as attachment
            viewer.Image = c.makeWebImage(Chart.PNG)
            viewer.StreamChart(fname & ".png", True)
        End If
    End If
